...
#HW 1
#AMPL model file for Problem 1-5a
#Steel Mill
...
#constraints:---------------------------------------------------
#Time constraint for each stage
subject to Time {s in STAGE}:
	#sum {p in PROD} (1/rate[p,s]) * Make[p] <= avail[s]; #original code
	sum {p in PROD} (1/rate[p,s]) * Make[p] = avail[s];   #modified version
...
printf "\n";
#printf "The maximum total profit from the original code: \n $%0.2f ", Total_Profit;
printf "The maximum total profit from the modified constraint code: \n $%0.2f ", Total_Profit;
printf "\n";