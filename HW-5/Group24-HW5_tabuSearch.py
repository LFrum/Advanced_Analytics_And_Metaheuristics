...
tabuTenure = 50 #time in tabu active period
counter = 0

while counter < tabuTenure: #time in tabu active period
    Neighborhood = neighborhood(x_curr)

    for s in Neighborhood:
        if s not in tabuList: #candidates
            solutionsChecked = solutionsChecked + 1
            countS = 0 #initial count
            for i in s: #count how many selection
                countS += i
            if countS > 23: #use if it's not tabu
                if evaluate(s)[0] > f_curr[0]:
                    x_curr = s[:]                 
                    f_curr = evaluate(s)[:]       
                    
                    if evaluate(s)[0] > f_best[0]: #update best
        ...
        tabuList.append(s) #add to tabu list
