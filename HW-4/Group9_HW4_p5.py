...
p = 0.55 #probability for using best improvement
pRandomWalk = 1-p #probability to do random walk

while done == 0:
    #random walk seed
    randomWalkSeed += 1000
    #random number generator
    pRandom = Random(randomWalkSeed)
    #find current p-value
    pCurrent = pRandom.uniform(0,1)
    ...
    # do a random walk when 1-p     
    elif pCurrent > pRandomWalk: 
        counter = 1
        for s in Neighborhood:   
           solutionsChecked = solutionsChecked + 1
           counter += 1
           #take the 10th neighbor 
           if counter == 10:
               #take the neighbor as the "best" solution
                x_best = s[:]                 #find the best member and keep track of that solution
                f_best = evaluate(s)[:]       #and store its evaluation
                #update the neighbor as the current solution 
                x_curr = x_best[:]         #else: move to the neighbor solution and continue
                f_curr = f_best[:]         #evalute the current solution     
...
print ("\nBest value found: {:0.2f}".format(f_best[0]))
