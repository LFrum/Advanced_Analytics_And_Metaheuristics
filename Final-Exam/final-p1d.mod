...
#decision variables-------------------------------------------------------------
var y {j in J} >= 1 integer; #total number of officers assigned at the segment
var S >=0;

#objective: minimum cost fuel plan-----------------------------------------
#maximize totalSpeeders: sum {j in J} y [j] * s[j];

#maximize minSpeeders: S;

minimize minSpeeders: S;
...
subject to setMinSpeeder: S >=  sum {j in J} y [j] * s[j];
#data-------------------------------------
data final-p1.dat;
...
printf "total minimum speeders for all segments: %3.0f \n", minSpeeders;
